...
<section class="stage">
  ...
  modelled residues were already inside budget — but 3KYS has a disordered stretch at
  230–238 with no density, so the target reaches RFD3 in <strong>more than one
  piece</strong>. The target is fixed conditioning rather than something
  ...
  superposes onto the deposited chain at <strong>0.11 Å</strong> Cα RMSD, so the pieces
  have not drifted apart. (This campaign saw three segments rather than two: benchmarking
  the trim afterwards showed it was deleting TEAD1's palmitoylated cysteine at 344,
  because that residue's name is not in gemmi's amino-acid table. Fixed — chain
  membership now follows the backbone, not the name — but this run predates it.)</p>
  <div class="two">
    <div>
