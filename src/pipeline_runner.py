...
def _stage_for_skill(skill_name: str) -> str:
    ...
    -> summary | enzyme_summary | binder_summary, protein-design-script ->
    design | enzyme_design).  Callers that know their stage must pass it
...
class PipelineRunner:
    ...
    def __init__(
        ...
        if workflow not in {"ppi", "enzyme", "binder"}:
            raise ValueError(
                f"Invalid workflow={workflow!r}; expected 'ppi', 'enzyme' or 'binder'."
            )
    ...
    def _run_enzyme_track(
        ...
        try:
            ...
            if start_idx <= 2:
                ...
                self._record_stage("theozyme_diagnose", "complete",
                                   result.stage_files["theozyme_diagnose"], diag,
                                   stage="theozyme_diagnose")
                if not auto_mode:
            ...
            if start_idx <= 3:
                ...
                if prior_pdbs and prior_pdbs.lower() not in ("none", "", "n/a"):
                    ...
                    self._record_stage("pdb-ligand-grafting", "skipped", stage="grafting")
    ...
    # ==================================================================
    # A third track alongside PPI and enzyme. The target is named by the user, so
    ...
    def _run_binder_track(
        ...
        *,
        ...
        Structured exactly like `_run_enzyme_track`: positional guards inside
        their own method, so `run()`'s PPI state machine is untouched.
        """
        from src.structure_trim import load_mapping
    ...
    _STAGE_CALL_PRIOR: dict[str, int] = {
        ...
        "design-analyst": 2,
        "enzyme-substrate-id": 6,
        "enzyme-active-site-modeling": 12,
        "pdb-ligand-grafting": 4,
        "enzyme-design-validation": 2,
    }
