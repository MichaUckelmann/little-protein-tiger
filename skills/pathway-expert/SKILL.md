...
description: >
  ...
  chimerax-ppi-analysis (provides PDB ID) and molecular-biology-expert (provides complex
...
analysis — that is the job of chimerax-ppi-analysis and molecular-biology-expert.

The output is a structured `## PATHWAY BIOLOGY REPORT` containing a recommended PPI
and PDB ID(s) ready to pass directly into chimerax-ppi-analysis.
...
- **Proposed next step**: Run chimerax-ppi-analysis on PDB <ID>, target chain = <protein>
...
- **→ chimerax-ppi-analysis**: use `Suggested PDB ID(s)` and `Target complex` as inputs
