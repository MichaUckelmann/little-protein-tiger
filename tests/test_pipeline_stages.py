...
def test_shared_skills_exist_and_make_the_inversion_ambiguous():
    ...
    assert "design-analyst" in shared               # summary | enzyme_ | binder_
    assert "protein-design-script" in shared        # design | enzyme_design
...
def test_no_stale_model_ids_in_the_config():
    ...
    for name in ("config.yaml", "config_enzyme_chemistry.yaml",
...
@pytest.mark.parametrize("workflow", ["ppi", "enzyme", "binder"])
